...
@router.get("/summary")
def get_summary(month: Optional[str] = Query(None), year: Optional[str] = Query(None), alltime: Optional[str] = Query(None)):
    ...
    if alltime is not None or (not month and not year and alltime == "1"):
        ...
        with _e.connect() as conn:
            ...
            months_row = conn.execute(_t("""
                SELECT COUNT(DISTINCT strftime('%Y-%m', date)) FROM transactions WHERE status != 'deleted'
            """)).fetchone()
    ...
    if year and not month:
        with _e.connect() as conn:
            row = conn.execute(_t("""
                SELECT SUM(CASE WHEN type='expense' THEN amount ELSE 0 END),
                       SUM(CASE WHEN type='income'  THEN amount ELSE 0 END), COUNT(*)
                FROM transactions WHERE strftime('%Y', date) = :y AND status != 'deleted'
            """), {"y": year}).fetchone()
        expense = float(row[0] or 0); income = float(row[1] or 0); tx_count = int(row[2] or 0)
        rate = round((income - expense) / income, 4) if income > 0 else 0.0
        return {"total_expense": expense, "total_income": income, "tx_count": tx_count,
    ...
    rate = savings_rate(month)

    today = date.today()

    # Compute active month and whether it's the current month
    ...
    if is_current_month:
        ...
        day_of_month = days_in_month  # past month: full month elapsed
        projected_expense = 0  # no projection needed for past months
    ...
    with _e2.connect() as _conn:
        _prev_row = _conn.execute(_t2(
            "SELECT COUNT(*) FROM transactions WHERE strftime('%Y-%m', date) = :prev_month AND status != 'deleted'"
        ), {"prev_month": _prev_month}).fetchone()
        _prev_expense_row = _conn.execute(_t2(
            "SELECT SUM(amount) FROM transactions WHERE strftime('%Y-%m', date) = :m AND type='expense' AND status != 'deleted'"
        ), {"m": _prev_month}).fetchone()
...
def get_monthly_trend(n_months: int = Query(6, ge=1, le=24)):
    rows = monthly_trend(n_months)
    return [
...
def get_categories(month: Optional[str] = Query(None), year: Optional[str] = Query(None), alltime: Optional[str] = Query(None)):
    from sqlalchemy import text as _t
    from src.data.db import engine as _e
    ...
    if alltime is not None:
        with _e.connect() as conn:
            rows = conn.execute(_t("""
                ...
                FROM transactions WHERE type='expense' AND status!='deleted'
    ...
    if year and not month:
        with _e.connect() as conn:
            rows = conn.execute(_t("""
                SELECT category, SUM(amount) as total
                FROM transactions WHERE type='expense' AND strftime('%Y',date)=:y AND status!='deleted'
                GROUP BY category ORDER BY total DESC
            """), {"y": year}).fetchall()
        return [{"category": r[0], "this_month": float(r[1] or 0), "last_month": 0.0} for r in rows]
...
@router.get("/forecast")
def get_forecast():
    """AI-powered 3-month spending forecast using Groq. Cached 24 hours."""
    import time
    from sqlalchemy import text as _t
    from src.data.db import engine as _e
    from src.llm.client import chat

    cached = _forecast_cache.get("forecast")
    if cached and time.time() - cached[0] < 86400:
        return cached[1]

    # Get all historical monthly expense totals
    with _e.connect() as conn:
    ...
    last_month = history[-1]["month"]  # YYYY-MM
    ...
    _forecast_cache["forecast"] = (time.time(), result)
    return result


@router.get("/top-merchants")
def get_top_merchants(month: Optional[str] = Query(None)):
    rows = top_merchants(month)
    return [
...
def get_health_score(month: Optional[str] = Query(None)):
    ...
    budget_rows = budget_vs_actual(month)
    ...
    trend = spending_trend_direction(3)
    ...
    # Add AI-generated tips for each sub-score
    try:
        from src.llm.client import chat
        import re as _re
        ai_prompt = (
...
def get_ai_summary(month: Optional[str] = Query(None)):
    """Groq-generated bullet-point summary of recent spending. Cached 1 hour."""
    import time
    from src.llm.client import chat

    cache_key = month or "current"
    cached = _summary_cache.get(cache_key)
    if cached and time.time() - cached[0] < 3600:
        return {"summary": cached[1], "cached": True}

    trend_rows = monthly_trend(3)
    cat_rows = category_comparison()
...
def ask_finances(body: dict):
    ...
    rows = recent_transactions(limit=50)
    tx_list = [
        {
            "date": str(r[1]),
            "merchant": r[3] or r[2],
